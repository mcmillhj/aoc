...
fun has_duplicates passphrase = let
    val words = String.tokens (fn c => c = #" ") passphrase
    fun has_duplicates' [] = false
      | has_duplicates' (x::xs) = List.exists (fn y => y = x) xs orelse has_duplicates' xs
in
    has_duplicates' words
end

fun has_anagrams passphrase = let
    val words = String.tokens (fn c => c = #" ") passphrase

    fun is_anagram a b = let
        val ascii_table = Array.tabulate(128, fn _ => 0)
    in
        CharVector.app (fn char => Array.update(ascii_table, ord char, Array.sub(ascii_table, ord char) + 1)) a;
        CharVector.app (fn char => Array.update(ascii_table, ord char, Array.sub(ascii_table, ord char) - 1)) b;

        Array.all (fn e => e = 0) ascii_table
    end

    fun has_anagrams' [] = false
      | has_anagrams' (x::xs) = List.exists (fn y => (is_anagram x y)) xs orelse has_anagrams' xs
in
    has_anagrams' words
end

fun valid_passphrase passphrase = not(has_anagrams passphrase) andalso not(has_duplicates passphrase)
...
List.length (List.filter (fn passphrase => (valid_passphrase passphrase)) passphrases)
