...
while ( my $instruction = <DATA> ) {
   ...
   my ($command, 
       ...
       $x2, 
       $y2,
   ) = $instruction =~ m/^(?:turn )?(\w+) (\d+),(\d+) through (\d+),(\d+)/;

   my $method = "light_${command}";
   __PACKAGE__->$method( points_in_rectangle($x1, $y1, $x2, $y2) );
}

say count_lit();

sub points_in_rectangle {
   my ($x1, $y1, $x2, $y2) = @_;
...
sub light_toggle {
   ...
   foreach my $point ( @points ) {
      my $index = 1024 * $point->[0] + $point->[1];
      vec($bitf, $index, 1) = abs(1 - vec($bitf, $index, 1));
   }
