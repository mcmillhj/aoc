...
sub is_valid_passport {
  my ($passport) = @_;

  foreach my $required_pattern (
    qr/byr:(19[2-9][0-9]|200[0-2])/,
         # (19[2-8][0-9]|199[0-9]|200[0-2]) 
    qr/iyr:(201[0-9]|2020)/,
         # (201[0-9]|2020)
    qr/eyr:(202[0-9]|2030)/,
         # (202[0-9]|2030)
    qr/hgt:(1[5-8][0-9]|19[0-3])cm\b|(59|6[0-9]|7[0-6])in\b/,
         # (1[5-8][0-9]|19[0-3])   (59|6[0-9]|7[0-6])
    qr/hcl:#[0-9a-f]{6}\b/,
    qr/ecl:(amb|blu|brn|gry|grn|hzl|oth)\b/,
    qr/pid:\d{9}\b/,
  ) {
    return 0 unless $passport =~ /$required_pattern/m;
  }
...
foreach my $passport (@passports) {
  next unless is_valid_passport($passport);
