...
# How many passwords are valid according to their policies?
  ...
  foreach my $line (@lines) {
    my ($min, $max, $policy_letter, $password) = $line =~ m/(\d+)-(\d+) (\w): (\w+)/;
    my $occurrences = () = $password =~ /$policy_letter/g;
    next unless $occurrences >= $min and $occurrences <= $max;
...
# How many passwords are valid according to the new interpretation of the policies?
  ...
  my $valid_passwords = 0;
  foreach my $line (@lines) {
    my ($first_pos, $second_pos, $policy_letter, $password) = $line =~ m/(\d+)-(\d+) (\w): (\w+)/;

    next unless (substr($password, $first_pos - 1, 1) eq $policy_letter) 
      xor (substr($password, $second_pos - 1, 1) eq $policy_letter);
