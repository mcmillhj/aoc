...
my %bags;
while (my $rule = readline(\*DATA)) {
  chomp($rule);

  my ($bag) = $rule =~ m/(.*?) bags contain/;
  my @contents = $rule =~ m/(\d+|no) (.*?) bags?/g;
  while (@contents) {
    my $size = shift @contents;
    ...
    push @{ $bags{$bag} }, { size => $size, type => $type };
  }
}
...
# How many bag colors can eventually contain at least one shiny gold bag? (The list of rules is quite long; make sure you get all of it.)
  ...
  sub find {
    ...
    my @containing_bags = @{ $bags{$bag} };
    return unless @containing_bags > 0;

    return
      grep { $_->{type} eq $target or find($_->{type}, $target) }
      @containing_bags;
  }
  ...
  foreach my $bag (keys %bags) {
    next if $bag eq $target;
    my (@bags) = find($bag, $target);
    next unless @bags;
...
# Of course, the actual rules have a small chance of going several levels deeper than this example; be sure to count all of the bags, even if the nesting becomes topologically impractical!
  ...
  sub count {
    ...
    my @containing_bags = @{ $bags{$target} };
    return 0 unless @containing_bags > 0;

    my $count = 0;
    foreach my $bag (@containing_bags) {
      $count += (count($bag->{type}) + 1) * $bag->{size};
    }

    return $count;
  }

  say count(qq{shiny gold});
}
